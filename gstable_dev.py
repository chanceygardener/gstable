...
def remove_non_numeric_chars(nstr):
	return ''.join([i for i in nstr if i.isnumeric()])
...
def colis(row, value, column): 
	if type(value) in (list, tuple):
		return True if row[column].strip() in value else False
	else:
...
class Load:
	...
	def add_tables(self, infiles, skip_columns_called=False, breakout=False):
		# make a hash with keys as output filenames and values as LoadTable objects derived from the input filenames
		add_ins = {outgen(out, self.ticket): LoadTable(out, self, skip_columns=skip_columns_called, breakout_set=breakout)
						for out in infiles}
...
class GsTable:
	...
	def tofile(self, fpath=None, delim='\t', encode_as='utf-8'):
		if not fpath is not None:
			fpath = self.outfile_name
		towork = self.table
		with open(fpath, 'w', encoding=encode_as) as file:
			name = self.infile_name.split('/')[-1]
			print('\n writing updated table from {} to file...\n'.format(name))
			file.writelines(delim.join(self.headers)+'\n')
			for row in towork:
				row = delim.join(str(val) for val in row)
				file.writelines(row+'\n')
		print('file saved to {}'.format(fpath))
...
class QATable(GsTable):
	def __init__(self,load, headed=True):
		...
		self.load = load
		self.infile_name = self.load.QA_file
...
class LoadTable(GsTable):
	...
	def __init__(self, infile_name, load, skip_columns, headed=True, breakout_set=False, from_array=False):
	...
	def dupe_scan(self, by_column):
		...
		for ind, row in enumerate(self):
			value = row[self.colhash[by_column]]
		...
		if len(dupes) != 0:
			...
			print('No duplicate {} values found in {}'.format(by_column, self.infile_name))
	...
	def split_by(self, condition, by_column=False):
		...
		self.load.tables[outgen(self.infile_name, self.load.ticket)] = tb1
		self.load.tables[outgen(self.infile_name, self.load.ticket)[:-4]+'_II.txt'] = tb2
		return tb1, tb2
	...
	def valtweak(self, tcol, tweaker, condition=None, headed=True):
	...
	def remove_rows_if(self, condition, tcol=None, testit=False, verbose=True):
		...
		self.table = output
		if verbose:
			print('\n{} rows removed from {}'.format(count, self.infile_name))
		return count
	
	def check_row(self, row_number):
		for head, ival in list(zip(self.headers, self.table[row_number])):
			print('{}: {}'.format(head, ival))
